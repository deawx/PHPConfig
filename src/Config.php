 ...
 *
 * @package    Config.php @ 2021-11-09T10:19:51.796Z
...
class Config
{
    ...
    {
        $return = false;
        if(isset(static::$objectConfig[$key])){
            $return = static::$objectConfig[$key];
        }
        return $return;
    }
     ...
     * @return void
     */
    public function setArray($name = null, array $configs = []): void
    {
        if(is_null($name)){
            static::$config = $configs;
            static::$objectConfig = [$this->convertObject($configs)];
        }else{
            if(static::$strtolower){
                $name = strtolower($name);
            }
            if(strpos($name, static::$delimiter)){
                $this->set($name, $configs);
            }else{
                static::$config[$name] = $configs;
                static::$objectConfig[$name] = $this->convertObject($configs);
            }
        }
    }
    ...
    public function setDir($name = null, string $path = __DIR__): bool
    {
        if(is_dir($path)){
            ...
            if(is_array($files)){
                ...
                $config_name_prefix = '';
                if(!is_null($name)){
                    $config_name_prefix = $name . static::$delimiter;
                }
    ...
    public function set(string $key, $value): self
    {
        ...
        if(strpos($key, static::$delimiter)){
            $keys = explode(static::$delimiter, $key);
            $id = $keys[0];
            array_shift($keys);
            $key = implode(static::$delimiter, $keys);
            if(!isset(static::$config[$id])){
                static::$config[$id] = [];
            }
            $set = $this->multiSubConfigSet($key, $value, static::$config[$id]);
            static::$config[$id] = $set;
            static::$objectConfig[$id] = $this->convertObject($set);
        }else{
            static::$config[$key] = $value;
            if(is_array($value)){
                static::$objectConfig[$key] = $this->convertObject($value);
            }else{
                static::$objectConfig[$key] = $value;
            }
        }
    ...
    protected function multiSubConfigSet($key, $value, $config): array
    {
        if(strpos($key, static::$delimiter)){
            ...
            $config[$id] = $this->multiSubConfigSet(implode(static::$delimiter, $keys), $value, $config[$id]);
        }else{
            $config[$key] = $value;
        }
